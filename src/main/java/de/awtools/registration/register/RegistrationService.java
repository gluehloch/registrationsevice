...
public class RegistrationService {
	...
	public DefaultRegistrationValidation registerNewAccount(RegistrationJson registration)
		...
		Optional<RegistrationEntity> registrationCheck = registrationRepository
				.findByNickname(registration.getNickname());
	...
	private RegistrationEntity createRegistration(RegistrationJson registration) {
		...
		/*
		 * TODO sendMailService.sendMail( "do-not-replay@tippdiekistebier.de",
		 * registration.getEmail().toString(), "Registrierung bestätigen.",
		 * "Confirm link..." );
		 */
	...
	public DefaultRegistrationValidation confirmAccount(Token token) throws RequestValidationException {
		...
		ApplicationEntity application = validateApplication(registration.getNickname(), registration.getApplication());
		...
		return new DefaultRegistrationValidation(registration.getNickname(), registration.getApplication(),
				Validation.ValidationCode.OK);
	}

	@Transactional
	public DefaultRegistrationValidation validate(String nickname, String email, String applicationName)
			throws RequestValidationException {
		...
		if (registrationDefined.isPresent()) {
			return new DefaultRegistrationValidation(nickname, applicationName,
					Validation.ValidationCode.KNOWN_NICKNAME);
		}

		registrationDefined = registrationRepository.findByEmail(Email.of(email));
		if (registrationDefined.isPresent()) {
			return new DefaultRegistrationValidation(nickname, applicationName,
					Validation.ValidationCode.KNOWN_MAIL_ADDRESS);
		}

		return new DefaultRegistrationValidation(nickname, applicationName, Validation.ValidationCode.OK);
	}

	private ApplicationEntity validateApplication(String nickname, String applicationName)
			throws RequestValidationException {

		ApplicationEntity application = applicationRepository.findByName(applicationName)
				.orElseThrow(() -> new RequestValidationException(new DefaultRegistrationValidation(nickname,
						applicationName, Validation.ValidationCode.UNKNOWN_APPLICATION)));
	...
	public DefaultRegistrationValidation createAccount(RegistrationJson registration) {
		...
		final UserAccountEntity userAccount = registrationRepository.findByNickname(registration.getNickname())
				.map(this::confirmAccount).map(this::createUserAccountEntity).orElseThrow();

		applicationRepository.findByName(registration.getApplicationName())
				.map(app -> this.addUserAccountToApplication(app, userAccount));

		return new DefaultRegistrationValidation(registration.getNickname(), registration.getApplicationName(),
				Validation.ValidationCode.OK);
	}
	...
	private UserAccountEntity addUserAccountToApplication(ApplicationEntity application,
